...
public:
  ...
  */
  HallSensor(GPIO_Value encA, GPIO_Value encB, GPIO_Value encC, int pp);

  /** HallSensor initialise pins */
  void init();

  //  HallSensor interrupt callback functions
  /** A, B, and C channel callback function */
  void handleCallback(uint16_t GPIO_Pin);

  // pins A and B
  GPIO_Value pinA; //!< HallSensor hardware pin A
  GPIO_Value pinB; //!< HallSensor hardware pin B
  GPIO_Value pinC; //!< HallSensor hardware pin C
...
private:
  ...
  volatile GPIO_PinState A_active;        //!< current active states of A channel
  volatile GPIO_PinState B_active;        //!< current active states of B channel
  volatile GPIO_PinState C_active;        //!< current active states of C channel
